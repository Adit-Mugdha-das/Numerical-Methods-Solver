...
def false_position_method(f_expr, a, b, tol=1e-6, max_iter=100):
    """Finds root using False Position Method with iteration tracking."""
    x = sp.symbols('x')
    ...
    iter_history = []  # Store iteration values

    for i in range(max_iter):
        c = (a * f(b) - b * f(a)) / (f(b) - f(a))
        iter_history.append(c)  # Store current midpoint

        if abs(f(c)) < tol:
            return c, iter_history  # Return root and all iterations
...
if __name__ == "__main__":
    try:
        ...
        root, history = false_position_method(f_expr, a, b, tol, max_iter)
        print(f"Root found at x = {root}")
        print("Iterations:", history)
